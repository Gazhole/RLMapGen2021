...
def gen_room_size(mode,room_max_size,direction_x=False,direction_y=False):
    ...
    if mode == "corridor":
        ...
        w = randint(6,room_max_size)
        h = randint(6,room_max_size)
...
def make_v_loops(m):
    ...
    for s_tile in s_walls:
        ...
        for n_tile in n_walls:
            ...
            if dist_between(sx, sy, nx, ny) <= 40:
                if sx == nx and line_of_dig(m, sx, sy, nx, ny):
                    if calc_dijkstra(m, sx, sy)[ny,nx] >= 100:
                        place_door(sx, sy, m)
                        place_door(nx, ny, m)
...
def make_h_loops(m):    
    ...
    for e_tile in e_walls:
        ...
        for w_tile in w_walls:
            ...
            if dist_between(ex, ey, wx, wy) <= 40:
                if ey == wy and line_of_dig(m, ex, ey, wx, wy):
                    if calc_dijkstra(m, ex, ey)[wy,wx] >= 100:
                        place_door(ex, ey, m)
                        place_door(wx, wy, m)
...
def get_cell(m,tx,ty):
    ...
    return np.array([[is_walkable(x, y, m) for y in range(ty-1,ty+2)] for x in range(tx-1,tx+2)])
...
class GameMap():
    def __init__(self,w,h):
        ...
        self.explored = np.zeros(shape=(h,w), dtype=int)
...
def get_visible_map_chunk(player, game_map, view_port_width, view_port_height):
    ...
    if map_chunk_x2 > game_map.w:
        map_chunk_x2 = game_map.w
        map_chunk_x1 = game_map.w - visible_width
    elif map_chunk_x1 < 0:
        map_chunk_x2 += -map_chunk_x1
        map_chunk_x1 = 0

    if map_chunk_y2 > game_map.h:
        map_chunk_y2 = game_map.h
        map_chunk_y1 = game_map.h - visible_height
    elif map_chunk_y1 < 0:
